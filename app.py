...
def create():
    db.create_all()
    # create default rooms if missing
...
def setup_admin():
    if User.query.filter_by(role="admin").first():
        flash("Admin already exists", "info"); return redirect(url_for("login"))
...
def register():
    if request.method=="POST":
        ...
        if not username:
            flash("Username required","danger"); return redirect(url_for("register"))
...
def login():
    if request.method=="POST":
        ...
        if user and user.check_password(password):
            login_user(user); flash("Logged in","success"); return redirect(url_for("index"))
...
def admin_panel():
    ...
    if request.method=="POST":
        ...
        if name and slug and not Room.query.filter_by(slug=slug).first():
            db.session.add(Room(name=name, slug=slug, created_by=current_user.id)); db.session.commit()
...
def make_mod():
    ...
    uid = int(request.form.get("user_id")); slug = request.form.get("room_slug")
    if uid and slug:
...
def on_private(data):
    to = data.get("to"); text = data.get("text"); frm = current_user.username
    if to and text:
        pm = PrivateMessage(sender=frm, recipient=to, text=text); db.session.add(pm); db.session.commit()
        # emit to recipient if online (find socket by room? here we broadcast personal event)
        emit("private_message", {"from": frm, "to": to, "text": text}, broadcast=True)

@socketio.on("kick")
def on_kick(data):
    # data: {"target":"username","room":"slug"}
    target = data.get("target"); room = data.get("room")
    # only admin or moderator of room can kick
    allowed = False
    if current_user.role == "admin":
        allowed = True
    else:
    ...
    if not allowed:
        ...
        return
    # broadcast kick notice (client side should handle actual disconnect if target matches)
    emit("kicked", {"target": target, "room": room}, room=room)
    # optional: record kick event (not persisted here)
