// Modern Chatly front-end script
...
function makeSocket(){
  ...
  socket.on('system',(s)=>{ const el=document.createElement('div'); el.className='system'; el.textContent=s.text; messagesEl.appendChild(el); messagesEl.scrollTop = messagesEl.scrollHeight; });
  socket.on('users',(users)=>{ /* could update users list */ });
  socket.on('rooms',(rooms)=>{ loadRooms(); });
}

async function initAfterAuth(){
  profileBlock.style.display='flex';
  const me = await api('/me'); if(!me.error){ meName.textContent = me.username; meXp.textContent = me.xp||1000; meLevel.textContent = me.level||Math.floor((me.xp||1000)/10); if(me.avatar) meAvatar.src = me.avatar; }
...
function renderMessage(m){
  ...
  if(m.from===username){
    const del = document.createElement('button'); del.className='delete-btn'; del.textContent='حذف'; del.onclick = async ()=>{
      const res = await fetch('/api/delete-message',{ method:'POST', headers: { 'Content-Type':'application/json', 'Authorization': token?('Bearer '+token):'' }, body: JSON.stringify({ timestamp: m.timestamp, room: m.room }) });
      const j = await res.json(); if(j.ok){ container.remove(); }
    };
